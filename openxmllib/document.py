...
class Document(object):
    ...
    def __init__(self, path_or_file_or_data_or_http, mime_type=None):
        """
        @param path_or_file_or_data_or_http: ... to the document
        @type path_or_file_or_data_or_http: a file path,
                                            a file object or
                                            a binary buffer or
                                            a http hosted file
        ...
        if type(path_or_file_or_data_or_http) in types.StringTypes:
            # Path or data
            try:
                if path_or_file_or_data_or_http.startswith('http://'):
                    filename = urllib.urlretrieve(path_or_file_or_data_or_http)[0]
                    self.filename = os.path.basename(filename)
                    in_file = file(filename, 'rb')
                elif os.path.isfile(path_or_file_or_data_or_http):
                    in_file = file(path_or_file_or_data_or_http, 'rb')
                    self.filename = os.path.basename(path_or_file_or_data_or_http)
                else:
                    raise TypeError
            except:
                in_file = cStringIO.StringIO(path_or_file_or_data_or_http)
                self.filename = None
        elif hasattr(path_or_file_or_data_or_http, 'read'):
            # File like object (supposed to)
            in_file = path_or_file_or_data_or_http
            try:
                self.filename = path_or_file_or_data_or_http.name
            except AttributeError, e:
                # A StringIO  or other unnamed stream
                self.filename = None
        else:
            raise TypeError, "Cannot use % documents" % str(path_or_file_or_data_or_http.__class__)
